...
export async function POST(req: Request) {
  try {
    ...
    if (!email || !name) {
      return NextResponse.json({ error: 'bad_request', message: 'Missing required fields: email, name' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: 'server_error', message: 'An unexpected error occurred while sending the welcome email.' }, { status: 500 });
  }
}
